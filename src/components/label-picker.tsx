    ...
    <Select.Root value={value} onValueChange={(next) => onValueChange(next as string | null)}>
      ...
      <Select.Portal>
        <Select.Positioner side="top" align="start" sideOffset={8} alignItemWithTrigger={false}>
          <Select.Popup className="min-w-40 max-h-96 overflow-y-auto bg-neutral-900 outline outline-neutral-800 rounded-xl p-1 shadow-lg data-starting-style:opacity-0 data-starting-style:scale-95 data-ending-style:opacity-0 data-ending-style:scale-95 transition-all duration-100 ease-out">
...
function LabelItem({ value, name }: { value: string | null; name: string }) {
  return (
    <Select.Item
      value={value}
      className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm text-neutral-200 cursor-default data-highlighted:bg-neutral-800"
    >
