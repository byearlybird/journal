...
export function createLabelRepo(db: Kysely<Database>) {
  return {
    ...
    create: async (label: Omit<LabelRow, "id" | "createdAt" | "updatedAt">, tx?: Transaction): Promise<string> => {
    ...
    update: async (id: string, label: Partial<Omit<LabelRow, "id" | "updatedAt">>, tx?: Transaction) => {
      const run = async (executor: Kysely<Database> | Transaction) => {
        const updatedAt = new Date().toISOString();
        await executor.updateTable("labels").set({ ...label, updatedAt }).where("id", "=", id).execute();
